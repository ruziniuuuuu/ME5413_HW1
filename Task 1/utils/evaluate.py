...
def visualize_distance_comparison(detections_tm, detections_tm_im, detections_kf, groundtruth, title="Distance Comparison Across Frames"):
    ...
    average_distance_tm = np.mean(distances_tm)
    average_distance_kf = np.mean(distances_kf)
    average_distance_tm_im = np.mean(distances_tm_im)
    plt.figure(figsize=(5, 5))
    ...
    plt.xlabel("Frame Number")
    plt.ylabel("Distance")
    plt.axhline(y=average_distance_tm, color='r', linestyle='--', label=f'Average Distance TM: {average_distance_tm:.2f}')
    plt.axhline(y=average_distance_kf, color='g', linestyle='--', label=f'Average Distance KF: {average_distance_kf:.2f}')
    plt.axhline(y=average_distance_tm_im, color='b', linestyle='--', label=f'Average Distance TM_IM: {average_distance_tm_im:.2f}')
    plt.grid()
    plt.legend()
    plt.show()